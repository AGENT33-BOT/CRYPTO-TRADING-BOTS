...
CONFIG = {
    'symbols': [
        # FIXED - Only proven performers (reduced from 10 pairs)
        'ETH/USDT:USDT',   # Proven performer
        'NEAR/USDT:USDT',  # Proven performer (high vol)
        'LINK/USDT:USDT',  # Added - 90% win rate
        'DOGE/USDT:USDT',  # Added - 100% win rate
        # REMOVED: BTC, SOL, XRP, AVAX, MATIC, ARB (causing losses)
    ],
    'timeframe': '1m',           # 1-minute candles for scalping
    'lookback': 50,
    'leverage': 5,               # Higher leverage for scalping
    'position_size_pct': 0.05,   # REDUCED to 5% for testing (was 10%) - spread risk across pairs
    'max_positions': 2,          # FIXED - Reduced from 5 (was testing too many)
    'test_mode': False,          # FIXED - Live mode now
    'add_to_dips': False,        # FIXED - Stop adding to losing positions
    'min_confidence': 85,        # INCREASED from 80
    
    # POSITION SIZING RULES - From analysis Feb 23, 2026
    'max_position_pct': 0.30,    # Max 30% of account per position (INCREASED)
